...
def compute_tf_idf(train_set, train_labels, dev_set):
    ...
    for movie in train_set:
        words_in_movie = set(movie)

        for w in words_in_movie:
            if docs_containing_word.__contains__(w):
                docs_containing_word[w] += 1
            else:
                docs_containing_word[w] = 1
    ...
    for movie in dev_set:
        word_count = Counter(movie)

        max_tfidf = 0
        max_word = ''

        for word in movie:
            tfidf = (word_count[word]/len(word_count)) * math.log(len(train_set)/(1 + docs_containing_word.get(word, 0)))
