...
# importing binary to ascii library
# Reference: https://docs.python.org/3/library/binascii.html
import binascii
...
class XSSEncodeAngles(object):
    ...
    def filter(input):
        ...
        # Refernce from: https://pynative.com/python-regex-replace-re-sub/
        filtered = re.sub("<", "&lt;", input)
...
class CSRFToken(object):
    ...
    def init(request, response):
        ...
        # Reference : https://stackoverflow.com/questions/31354485/how-to-generate-16-bytes-random-number-in-python-and-xor-them#:~:text=generating%20random%20bytes%3A,will%20output%2016%20random%20bytes.
        if(token == None):
            token = str(binascii.b2a_hex(token_bytes(16)))
            token = token[2:34]
