/*
In this MP we have implemented a floor planner to achieve area efficient floor plan. We have used
a binary tree data structure to implement this whose nodes can either be a module or a cutline.
We have used recursion throughout the methods for efficient peformance.

partners: danahar2, sg49, ambala2
*/
...
int is_leaf_node(node_t* ptr) {
  ...
  //condition for a node to be a leaf node
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  //a does not exist and base case condition
  if(a == NULL) {
    return 0;
  } else if(a == b) {
    //found subtree a and b are equal
    return 1;
  }
...
void rotate(node_t* ptr) {
  ...
  //init temp variable for swapping
  int temp;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // initializing memory for temp variable
  node_t *temp = (node_t *)malloc(sizeof(node_t));

  //swapping the parents of node a and node b using temp
  temp = a->parent;
  a->parent = b->parent;
  b->parent = temp;

  //checking if the subtree being switched is on the left or the right
  //side of the parent node and handling them respectively
  if(a->parent->left == b) {
    a->parent->left = a;
  } else {
    a->parent->right = a;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  //handling the cutlines
  //pushing the cutline to the array if it is V or H
  ...
  //incrementing the array index nth
  *nth = *nth + 1;
}
...
int get_total_resource(node_t* ptr)
{
  ...
  //recursive call
  return get_total_resource(ptr->right) + get_total_resource(ptr->left);
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  //initializing the memory for a new node
  node_t *temp = (node_t *)malloc(sizeof(node_t));

  //base case for recursion
  if(n == num_modules - 1){

    //assigning the data to the module pointed by temp
    temp->module = modules + n;
    temp->cutline = UNDEFINED_CUTLINE;
    temp->parent = par;
    return temp;
  }
  ...
  //assigning the data to the module pointed by temp->right
  temp->right->module = modules + n;
  temp->right->cutline = UNDEFINED_CUTLINE;
  temp->right->parent = temp;

  //Left child handling
  temp->left = init_slicing_tree(temp, n + 1);
