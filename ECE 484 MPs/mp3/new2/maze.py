...
class Maze(object):
    ...
    def sensor_model(self, coordinates, sensor_limit, orientation = 0):
        ...
        pos_y = y
        d5 = 0
        dx = np.cos(orientation-np.pi/4)
        dy = np.sin(orientation-np.pi/4)
        ...
        pos_y = y
        d6 = 0
        dx = np.cos(orientation-3*np.pi/4)
        dy = np.sin(orientation-3*np.pi/4)
        ...
        # Measure distance between wall and vehicle in front left direction
        pos_x = x
        pos_y = y
        d8 = 0
        dx = np.cos(orientation+np.pi/4)
        ...
        # return [d1*100, d5 * 100, d2*100, d6*100, d3*100, d7*100, d4*100, d8*100]
