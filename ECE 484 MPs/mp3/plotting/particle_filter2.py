...
class particleFilter:
    ...
    def runFilter(self):
        ...
        pos_npy_filename = f"pos_{num_particles}_{sensor_distance}"
        ori_npy_filename = f"ori_{num_particles}_{sensor_distance}"
        pos_array = None
        ori_array = None

        if os.path.isfile(pos_npy_filename):
            with open(pos_npy_filename, "rb") as f:
                pos_array = np.load(f)
        if os.path.isfile(ori_npy_filename):
            with open(ori_npy_filename, "rb") as f:
                ori_array = np.load(f)
        ...
        while True:
            ...
            if iterations == 100:
                ...
                if pos_array is not None:
                    pos_array.append(np.array(pos_error))
                else:
                    pos_array = np.array(pos_error)

                if ori_array is not None:
                    ori_array.append(np.array(ori_error))
                else:
                    ori_array = np.array(ori_error)
                
                with open(pos_npy_filename, "wb") as f:
                    pos_array = np.save(f, pos_array)
                with open(ori_npy_filename, "wb") as f:
                    ori_array = np.save(f, ori_array)
                exit(0)
