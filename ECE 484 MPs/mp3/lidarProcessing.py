...
class LidarProcessing:
    ...
    def construct_birds_eye_view(self, data):
        ...
        diag_error = 2 * np.cos(np.pi / 4) * 0.1
        # Handle sensor at 4 diagnal direction
        # Getting sensor reading for front left
        filter_front_left = np.logical_and(np.ones(y_points.shape), (abs(y_points - x_points) < diag_error))
        ...
        filter_rear_left = np.logical_and(np.ones(y_points.shape), (abs(y_points - x_points) < diag_error))
        ...
        filter_front_right = np.logical_and(np.ones(y_points.shape), abs(y_points - x_points) < diag_error)
        ...
        filter_rear_right = np.logical_and(np.ones(y_points.shape), (abs(y_points - x_points) < diag_error))
    ...
    def processLidar(self):
        ...
        return [front*100, front_right* 100, right*100, rear_right * 100, rear*100, rear_left * 100, left*100, front_left * 100]
