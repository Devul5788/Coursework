...
def run_model():
    ...
    def shutdown():
        ...
        # print("here1", positionList)
        # temp = np.array(pos_list)
        # # print("here1", temp[0:1][:])
        # newList = []
        # for entry in temp:
        #     newList.append([entry[0], entry[1]])
        # print(newList)
        # with open(r'/home/aar8/Desktop/waypoints.txt', 'w') as fp:
        #     for item in newList:
        #         fp.write("%s\n" % item)
        # plt.plot(positionList, pos_list[:][:])
        # print(type(waypoints.getWayPoints()[0:1]))
    ...
    while not rospy.is_shutdown():
        ...
        controller.execute(currState, [ref_x, ref_y, target_orientation, target_v], positionList)
    # pickle.dump([waypoint_list, trajectory], open('filepath', 'wb'))
    # loaded_data = pickle.load(open(, 'rb'))
    # print(positionList)
    # print("here", WayPoints.getWayPoints(waypoint_list))
    # plt.plot(positionList, WayPoints.getWayPoints(waypoint_list))
