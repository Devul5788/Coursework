...
def line_fit(binary_warped):
	...
	for window in range(nwindows):
		...
		##TO DO
		top_left = (int((leftx_current - margin)), int((binary_warped.shape[0] - (window+1) * window_height)))
		bottom_right = (int((leftx_current + margin)), int((binary_warped.shape[0] - window * window_height)))

		top_left2 = (int((rightx_current - margin)), int((binary_warped.shape[0] - (window+1) * window_height)))
		bottom_right2 = (int((rightx_current + margin)), int((binary_warped.shape[0] - window * window_height)))
		...
		for i in range(len(nonzerox)):
			...
			if ((x >= top_left[0] and x <= bottom_right[0]) and (y >= top_left[1] and y <= bottom_right[1])):
				...
				store_val_l.append(x)

			if ((x >= top_left2[0] and x <= bottom_right2[0]) and (y >= top_left2[1] and y <= bottom_right2[1])):
