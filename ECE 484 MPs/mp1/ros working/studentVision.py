...
class lanenet_detector():
	...
	def img_callback(self, data):
		...
		# img_birdeye, M, Minv = self.perspective_transform(raw_img[:,:,1])
	...
	def color_thresh(self, img, thresh=(140, 255)):
		"""
		...
		binary_output_l[(l_channel >= thresh[0]) & (l_channel <= thresh[1])] = 255

		#Hint: threshold on H to remove green grass (green has hue between 81 and 140)
		# h_channel = hls_img[:,:,0]
		# binary_output_h = np.zeros_like(h_channel)
		# binary_output_h[(h_channel < 80) | (h_channel > 150)] = 255

		# binary_output = binary_output_l & binary_output_h
	...
	def combinedBinaryImage(self, img):
		...
		## TODO
		SobelOutput = self.gradient_thresh(img, 125, 150)
		ColorOutput = self.color_thresh(img, (175, 255))

		####

		binaryImage = np.zeros_like(SobelOutput)
		binaryImage[(ColorOutput==255) | (SobelOutput==255)] = 1
	...
	def detection(self, img):
		...
		# print(self.combinedBinaryImage(img))
		img_birdeye, M, Minv = self.perspective_transform(binary_img)
		# cv_image = self.bridge.imgmsg_to_cv2(img_birdeye, "bgr8")
		# cv2.imshow(cv_image)
