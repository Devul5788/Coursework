...
def line_fit(binary_warped):
	...
	for window in range(nwindows):
		...
		##TO DO
		top_left = ((leftx_current - margin),(binary_warped.shape[0] - (window+1) * window_height))
		bottom_right = ((leftx_current + margin),(binary_warped.shape[0] - window * window_height))

		top_left2 = ((rightx_current - margin),(binary_warped.shape[0] - (window+1) * window_height))
		bottom_right2 = ((rightx_current + margin),(binary_warped.shape[0] - window * window_height))

		####
		# Draw the windows on the visualization image using cv2.rectangle()
		##TO DO
		color = (0,0,255) #BGR
		out_img = cv2.rectangle(out_img, top_left, bottom_right, color, 2)
		out_img = cv2.rectangle(out_img, top_left2, bottom_right2, color, 2)
		...
		for i in range(len(nonzerox)):
			if ((nonzerox[i] >= top_left[0] and nonzerox[i] <= top_left[0] + margin * 2) and (nonzeroy[i] >= bottom_right[0] - window_height and nonzeroy[i] <= bottom_right[0])):
				left_lane_inds.append(i)
				store_val_l.append(nonzerox[i])

			if ((nonzerox[i] >= top_left2[0] and nonzerox[i] <= top_left2[0] + margin * 2) and (nonzeroy[i] >= bottom_right2[0] - window_height and nonzeroy[i] <= bottom_right2[0])):
				right_lane_inds.append(i)
				store_val_r.append(nonzerox[i])
		...
		##TO DO
		if (len(store_val_l) > minpix):
			leftx_current = np.mean(store_val_l)
		
		if (len(store_val_r) > minpix):
			rightx_current = np.mean(store_val_r)
