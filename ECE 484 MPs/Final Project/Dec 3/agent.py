...
class Agent():
    ...
    def object_detection(self, throttle, brake, steer, filtered_obstacles, transform, boundary, vel):
        ...
        for i in range(len(filtered_obstacles)):
            ...
            if (passed_objects[sorted_min_left_dists[0][7]] == 0 and passed_objects[sorted_min_right_dists[0][7]] == 0 and sorted_min_left_dists[0][0] > sorted_min_right_dists[0][0]):
                print("took left")
                avg_x = (sorted_min_left_dists[0][3] + left[sorted_min_left_dists[0][5]].transform.location.x)/2 #If object is not passed, sum x-coord of left-most obstacle and closest left waypoint's x
                avg_y = (sorted_min_left_dists[0][4] + left[sorted_min_left_dists[0][5]].transform.location.y)/2
                ...
                # obstacle_vel = filtered_obstacles[sorted_min_left_dists[0][7]].get_velocity()
                # obstacle_velocity = np.sqrt(obstacle_vel.x**2 + obstacle_vel.y**2 + obstacle_vel.z**2)

                # if (obstacle_velocity > 0):
                #     return (0.1, 0, angle - yaw)

                # print(abs(angle - yaw))
                # if (np.abs(max(angle, yaw) - min(angle, yaw)) < 0.1):
                #     return (0.3, 0.3, angle - yaw)
                # if (passed_objects[sorted_min_right_dists[0][7]] == 0 and sorted_min_right_dists[0][0] >= vehicle_size):
                #     print("took right after left")
                #     avg_x = (sorted_min_right_dists[0][3] + right[sorted_min_right_dists[0][5]].transform.location.x)/2 #If object is not passed, sum x-coord of right-most obstacle and closest right waypoint's x
                #     avg_y = (sorted_min_right_dists[0][4] + right[sorted_min_right_dists[0][5]].transform.location.y)/2
                #     diff_y = avg_y - transform.location.y #Calculating the difference between the average x and y and the current location of vechicle
                #     diff_x = avg_x - transform.location.x
                #     angle = np.arctan2(diff_y , diff_x) #Calculating angle and yaw
                #     yaw = transform.rotation.yaw * np.pi/180
            ...
            # elif space in middle:
            ...
            elif (passed_objects[sorted_min_left_dists[0][7]] == 0 and passed_objects[sorted_min_right_dists[0][7]] == 0 and sorted_min_right_dists[0][0] > sorted_min_left_dists[0][0]):
                print("took right")
                avg_x = (sorted_min_right_dists[0][3] + right[sorted_min_right_dists[0][5]].transform.location.x)/2 #If object is not passed, sum x-coord of right-most obstacle and closest right waypoint's x
                avg_y = (sorted_min_right_dists[0][4] + right[sorted_min_right_dists[0][5]].transform.location.y)/2
                diff_y = avg_y - transform.location.y #Calculating the difference between the average x and y and the current location of vechicle
                ...
                # if (np.abs(max(angle, yaw) - min(angle, yaw)) < 0.1):
                #     return (0.3, 0.3, angle - yaw)
                # if (passed_objects[sorted_min_left_dists[0][7]] == 0 and sorted_min_left_dists[0][0] >= vehicle_size):
                #     print("took left after right")
                #     avg_x = (sorted_min_left_dists[0][3] + left[sorted_min_left_dists[0][5]].transform.location.x)/2 #If object is not passed, sum x-coord of left-most obstacle and closest left waypoint's x
                #     avg_y = (sorted_min_left_dists[0][4] + left[sorted_min_left_dists[0][5]].transform.location.y)/2
                #     diff_y = avg_y - transform.location.y #Calculating the difference between the average x and y and the current location of vechicle
                #     diff_x = avg_x - transform.location.x
