    ...
    def object_detection(self, throttle, brake, steer, filtered_obstacles, transform, boundary):
        ...
        for i in range(len(filtered_obstacles)): #For all obstacles in filtered object, check if distance less than 15
            ...
            for j in range(len(left)): #For each left boundary waypoint, find the distance to the left and right boundaries from the obstacle
                ...
                if dist < min_dist_l: #If distance is less than min left/right distance (100 initially), set it equal to new min_dist value, and rewrite min left/right lists to:
                    min_dist = dist_l
                    min_left_dists[i] = (dist_l, diff_y_l, diff_x_l, x_cord, y_cord, j) #Distance to left boundary, x&y diffs between obstacle and boundary, x&y coords, and the closest waypoint index?
        ...
        print("\n")
        sorted(min_left_dists, key = lambda x: x[0]) #Sort the min left dist list?
        print(min_left_dists)
        print("\n")

        # return(1, 0, steer)
        
        if min_left_dists[0][0] >= 3: #If smallest left distance is greater than 3 (considered car width)
            avg_x = (min_left_dists[0][3] + left[min_left_dists[0][5]].transform.location.x)/2 #summing x-coord of left-most obstacle and closest left waypoint's x
            ...
            yaw = transform.rotation.yaw * np.pi/180 #Calculating angle and yaw
