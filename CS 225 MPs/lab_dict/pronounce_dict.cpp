...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(myWord1.begin(), myWord1.end(), myWord1.begin(), ::toupper);
    std::transform(myWord2.begin(), myWord2.end(), myWord2.begin(), ::toupper);
   
    map<std::string, std::vector<std::string>>::const_iterator lookup = dict.find(myWord1);
    map<std::string, std::vector<std::string>>::const_iterator lookup2 = dict.find(myWord2);
   
    if(lookup == dict.end() || lookup2 == dict.end()) return false;
    if(lookup->second.size() != lookup2->second.size()) return false;
    else{
      for(size_t i = 0; i < lookup->second.size(); i++){
        if(lookup->second[i] != lookup2->second[i]) return false;
      }
    }
