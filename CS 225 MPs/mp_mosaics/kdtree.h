...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* buildTree(vector<Point<Dim>>& newPoints, int dim, int left, int right);

    int partition(vector<Point<Dim>>& newPoints, int dim, int left, int right, int pivotIndex);
    
    double squaredDist(const Point<Dim>& p1, const Point<Dim>& p2) const;
    ...
    Point<Dim> select(vector<Point<Dim>>& newPoints, int dim, int left, int right, int k);
};
