...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    unsigned int myStep = hashes::secondary_hash(key, size);
    unsigned int myIndex = hashes::hash(key, size);

    while(table[myIndex] != NULL) myIndex = (myIndex + myStep) % size;
    
    table[myIndex] = new std::pair<K, V>(key,value);
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    while(should_probe[myIndex]) {
        ...
        myIndex = (myIndex + myStep) % size;

        if(myString == myIndex) break;
    }
