                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.size() == 0 && mergeQueue.size() == 0) {return NULL;}
		...
		TreeNode * tmp = mergeQueue.front();
		mergeQueue.pop();
		return tmp;
	} else if (mergeQueue.size() == 0) {
		TreeNode * tmp = singleQueue.front();
		singleQueue.pop();
		return tmp;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() != 1) {
    	TreeNode * small = removeSmallest(singleQueue, mergeQueue);
    	TreeNode * nextSmall = removeSmallest(singleQueue, mergeQueue);

    	TreeNode * mergedTree = new TreeNode(small->freq.getFrequency() + nextSmall->freq.getFrequency());
    	mergedTree->left = small;
    	mergedTree->right = nextSmall;
    	mergeQueue.push(mergedTree);
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
	...
	if (!bfile.hasBits()) return NULL;
		...
		Frequency new_freq(bfile.getNextByte(), 0);
		newNode = new TreeNode(new_freq);
	}
	else {
		Frequency new_freq(0);
		newNode = new TreeNode(new_freq);
