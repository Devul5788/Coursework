...
size_t bsearch(const std::vector<T>& elements, const C& val, unsigned left, unsigned right){
	if (right - left == 0) {
        if(val > elements[right]) return left = right + 1;
        else return right;
    } 
	unsigned int mid = (right + left)/2;
	if (elements[mid] > val) return bsearch(elements, val, left, mid);
	if (elements[mid] < val) return bsearch(elements, val, mid + 1, right);
	else return mid;
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val){
	...
	return bsearch(elements, val, 0, elements.size() - 1);
}
