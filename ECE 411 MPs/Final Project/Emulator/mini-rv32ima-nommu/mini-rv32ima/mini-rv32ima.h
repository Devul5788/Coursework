...
MINIRV32_DECORATE int32_t MiniRV32IMAStep( struct MiniRV32IMAState * state, uint8_t * image, uint32_t vProcAddress, uint32_t elapsedUs, int count )
{
	...
	for( int icount = 0; icount < count; icount++ )
	{
		...
		{
			trap = 1 + 0;  // Instruction address misaligned
			break;
		}
		else
		{	
			...
			switch( ir & 0x7f )
			{
				...
				case 0b1100011: // Branch (pc←pc+((rs1 {math op} rs2)?immb:4))
				{
					...
					switch( ( ir >> 12 ) & 0x7 )
					{
						...
						default: trap = (2+1);												//Illegal instruction
					}
					break;
				}
				case 0b0000011: // Load 
				...
				// lhu -> rd←zx(m16(rs1+immi)),pc←pc+4
				{
					...
					**/
					...
					if( rsval >= MINI_RV32_RAM_SIZE-3 )
					{
						...
						{
							trap = (5+1);	//Load address misaligned
							rval = rsval;
						}
					}
					...
					else
					{
						switch( ( ir >> 12 ) & 0x7 )
						{
							case 0b000: rval = (int8_t)MINIRV32_LOAD1( rsval ); break;	//LB
							case 0b001: rval = (int16_t)MINIRV32_LOAD2( rsval ); break;	//LH
							case 0b010: rval = MINIRV32_LOAD4( rsval ); break;			//LW
							case 0b100: rval = MINIRV32_LOAD1( rsval ); break;			//LBU
							case 0b101: rval = MINIRV32_LOAD2( rsval ); break;			//LHU
							default: trap = (2+1);										//Illegal instruction
						}
					}
					break;
				...
				case 0b0100011: // Store
				{
					...
					else
					{
						switch( ( ir >> 12 ) & 0x7 )
						{
							case 0b000: MINIRV32_STORE1( addy, rs2 ); break;	//SB
							case 0b001: MINIRV32_STORE2( addy, rs2 ); break;	//SH
							case 0b010: MINIRV32_STORE4( addy, rs2 ); break;	//SW
							default: trap = (2+1);								//Illegal instruction
						}
					}
					break;
				...
				case 0b1110011: // Zifencei+Zicsr
				{
					...
					else if( microop == 0b000 ) // "SYSTEM" 
					{
						...
						else
						{
							...
							switch( csrno )
							{
								// ECALL (trap to debugger); 8 = "Environment call from U-mode"; 11 = "Environment call from M-mode"
								case 0: trap = ( CSR( extraflags ) & 3) ? (11+1) : (8+1); break; 
								// EBREAK 3 = "Trap to OS"
								case 1:	trap = (3+1); break;  //Breakpoint
								default: trap = (2+1); break; // Illegal opcode.
							}
						}
					}
					else
						trap = (2+1); 				// Illegal instruction -> Note micrrop 0b100 == undefined.
					break;
				}
				case 0b0101111: // RV32A -> https://msyksphinz-self.github.io/riscv-isadoc/html/rva.html
				{
					...
					else
					{
						...
						switch( irmid )
						{
							...
							default: trap = (2+1); dowrite = 0; break; 							//Illegal instruction
						}
