...
restart:
	{
		...
		// Goes to the end of the file, gets the length (checks if size can fit in RAM), and then goes back to the beginning
		fseek( f, 0, SEEK_END );
		long flen = ftell( f );
		fseek( f, 0, SEEK_SET );
		if( flen > ram_amt )
		...
		// load the image into ram_image array and then if successful close the file. 
	...
	// 		dtb[0x13c/4] = validram_addr;
	// 		// "memory" property of the DTB (e
	// 	}
	// }
