...
restart:
	{
		...
		if(fread(ram_image + 0x400000, image_len, 1, image_file) != 1)
		{
		...
		if( dtb_file_name )
		{
			...
			else
			{
				...
				dtb_ptr = ram_amt - dtblen - sizeof( struct MiniRV32IMAState ) - 1;
				// 1 object read -> DTB
	...
	core->mideleg = 0x1666; // required by qemu
	// core->medeleg = 0x8; // required by qemu
	...
	// instct+1 || instct < 0
	// rt < instct+1 || instct < 0
	// 3000000
	for( rt = 0; rt < 3500000; rt += instrs_per_flip )
	{
		...
		// printf("rt: %d\n ", rt);
		// printf("pc: %x\n\n", core->pc);
		// rt > 2943181 && rt < 2943186
		if( single_step && rt > 2943181) {
			printf("rt: %ld\n", rt - 2943182);
			DumpState( core, ram_image);
		}

		// execute instructions
		int ret = MiniRV32IMAStep( core, ram_image, 0, elapsedUs, instrs_per_flip, rt, fw_len, dtb_ptr); // Execute upto 1024 cycles before breaking out.
		switch( ret )
		{	
