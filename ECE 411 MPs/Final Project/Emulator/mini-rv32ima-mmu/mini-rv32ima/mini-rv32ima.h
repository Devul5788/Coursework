...
MINIRV32_DECORATE int32_t MiniRV32IMAStep( struct MiniRV32IMAState * state, uint8_t * image, uint32_t vProcAddress, uint32_t elapsedUs, int count, uint32_t rt, uint32_t fw_len, uint32_t dtb_ptr);
...
uint64_t mmu_translate(struct MiniRV32IMAState * state, uint32_t vpn_addr, uint32_t access_type, uint8_t * image, uint32_t * trap) {
	...
	// printf("\nfinally mmu being used\n");
	// printf("addr: %d\n", vpn_addr);
	// return vpn_addr;
	// printf("mprv_priv: %x curr_priv: %x access_type: %x\n", mprv_priv, curr_priv, access_type);
	// printf("vpn addr: %d\n", vpn_addr);
	...
	for (uint im = 0; im < 2; im++) {
        uint page_addr = im == 0 ?
            (ppn * PAGESIZE + ADDR_PART_PN1(vpn_addr) * PTESIZE) :
            ((page.ppn0 | (page.ppn1 << 10)) * PAGESIZE + ADDR_PART_PN0(vpn_addr) * PTESIZE);
        page = load_page(page_addr, image);
        super = im == 0;
    ...
    uint8_t perm =
        mprv_priv == PRIV_MACHINE || // machine can read everything
        (mprv_priv == PRIV_USER && page.u) || // this is a user page
        (mprv_priv == PRIV_SUPERVISOR && (!page.u || sum)); // supervisor page or SUM
    uint8_t access =
        (mode == EXECUTE && page.x) ||
        (mode == LOAD && (page.r || (page.x && mxr))) ||
        (mode == STORE && page.w);
    uint8_t allowed = perm && access;
    ...
    // translation success
    uint pa = ADDR_PART_OFFSET(vpn_addr);
    pa |= super ? ADDR_PART_PN0(vpn_addr) << 12 : page.ppn0 << 12;
    pa |= page.ppn1 << 22;
	...
	// for (uint im = 0; im < 2; im++) {
	// 	uint32_t addr_part_pn0 = (vpn_addr >> 12) & 0x3ff;
	// 	uint32_t addr_part_pn1 = (vpn_addr >> 22) & 0xfff;
    //     uint32_t pte_addr = im == 0 ?
    //         (ppn * PAGESIZE + addr_part_pn1 * PTESIZE) :
    //         ((page.ppn0 | (page.ppn1 << 10)) * PAGESIZE + addr_part_pn0 * PTESIZE);
	// 	printf("pte_addr rvc: %x\n", pte_addr);
		
    //     page = load_page(pte_addr, image);
    //     // super = im == 0;
	...
	// 	uint32_t pt_idx = (vpn_addr >> (12 + 10 * i)) & 0x3FF;
    //     uint32_t pte_addr = page_table_addr + pt_idx * PTESIZE;
	// 	printf("pte_addr my: %x\n", pte_addr);
...
MINIRV32_DECORATE int32_t MiniRV32IMAStep( struct MiniRV32IMAState * state, uint8_t * image, uint32_t vProcAddress, uint32_t elapsedUs, int count, uint32_t rt, uint32_t fw_len, uint32_t dtb_ptr)
{
	...
	for( int icount = 0; icount < count; icount++ )
	{
		...
		int mte = 0; // https://github.com/riscv/riscv-j-extension/blob/master/pointer-masking-proposal.adoc
		...
		{	
			...
			switch( ir & 0x7f )
			{
				...
				// lhu -> rd←zx(m16(rs1+immi)),pc←pc+4
				{
					...
					**/
					...
					if( rsval >= MINI_RV32_RAM_SIZE-3 )
					{
						...
						if( (rsval >= 0x02000000  && rsval <= 0x0200bffc) || rsval == 0x10000005 || rsval == 0x10000000 ) 
						{
					...
					{
						// if (rsval > fw_len && rsval < 0x400000){
						// 	rval = 0;
						// 	break;
						// }

						// if (rsval > 0x400000 && rsval < dtb_ptr &){
						// 	printf("rsval1: %x", rsval);
						// 	rsval = rsval - 0x400000 + fw_len;
						// 	printf("rsval2: %x", rsval);
						// }
						uint64_t paddr = mmu_translate(state, rsval, LOAD, image, &trap);
						// printf("reg: %d\n", rdid);
						// printf("load: %x\n", paddr);
						// printf("value: %x\n", MINIRV32_LOAD4( paddr ));
						// printf("rs1: %x\n", rs1);
						// printf("imm_se: %d\n", imm_se);

						// if(paddr == 0x1e02c){
						// 	printf("value at broken load: %d", image[0x1e02c]);
						// }
				...
				case 0b0100011: // Store
				{
					...
					if( addy >= MINI_RV32_RAM_SIZE-3 )
					{
						...
						if((addy >= 0x02000000  && addy <= 0x0200bffc) || addy == 0x100000 || addy == 0x10000000 )
						{
					...
					else
					{
						...
						// if(paddr == 0x1e02c){
						// 	printf("value at broken store: %d", image[0x1e02c]);
						// }
					}
					break;
				}
				...
				case 0b1110011: // Zifencei+Zicsr
				{
					...
					if( (microop & 3) ) // It's a Zicsr function.
					{
						...
						switch( csrno )
						{
							...
							case 0x301: rval = 0x401411ad; break; //machine ISA reg (XLEN=32, IMA+X) -> https://five-embeddev.com/riscv-isa-manual/latest/machine.html#sec:misa
							case 0x303: rval = 0x1666; break;     //mideleg
							...
							case 0x3c0: rval = 0; mte = 1; break; //mte
							case 0x100: rval = CSR( mstatus ) & 0x000de162; break; //sstatus
							case 0x104: rval = CSR( mie ) & 0x222; break; //sie
							case 0x144: rval = CSR( mip ) & 0x222; break; //sip
						...
						// printf("mstatus: %x for pc: %x for rt: %d\n", CSR( mstatus ), pc, rt);
						switch( csrno )
						{ 
							...
							case 0x303: SETCSR( mideleg, writeval); break; // maybe & 0x666 (qemu)
							...
							case 0x100: //sstatus
								uint32_t mstatus_val = CSR( mstatus );  
								mstatus_val &= ~0x000de162;
								mstatus_val |= writeval & 0x000de162;
								SETCSR( mstatus, mstatus_val );
								break; 
							case 0x104: //sie
								uint32_t mie_val = CSR( mie );  
								mie_val &= ~0x222;
								mie_val |= writeval & 0x222;
								SETCSR( mie, mie_val );
								break;
							case 0x144: //sip
								uint32_t mip_val = CSR( mip );  
								mip_val &= ~0x222;
								mip_val |= writeval & 0x222;
								SETCSR( mip, mip_val );
								break;
					...
					else if( microop == 0b000 ) // "SYSTEM" 
					{
						...
						else if( ( ( csrno & 0xff ) == 0x02 ) )  // MRET (return from traps, exceptions, interrupts)
						{
							...
							if (csrno >> 8 == 0x3 ) { //mret
								uint32_t mstatus = CSR( mstatus );
								uint32_t flags = CSR( extraflags );
								flags &= ~0x3; // Clear bits 0 and 1
								uint32_t mpie = (mstatus >> 7) & 1;
								uint32_t mpp = (mstatus >> 11) & 0x3;
								flags |= mpp & 0x3; // Set bits 0 and 1 to the corresponding bits in mpp
								uint32_t mprv = mpp == PRIV_MACHINE ? ((mstatus >> 17) & 1) : 0;
								uint32_t new_mstatus = (mstatus & ~0x21888) | (mprv << 17) | (mpie << 3) | (1 << 7);
								SETCSR(mstatus, new_mstatus);
								SETCSR(extraflags, flags);
								pc = CSR( mepc ) -4;
							} else if (csrno >> 8 == 0x1 ) {
								uint sstatus = CSR( mstatus ) & 0x000de162; //sstatus is a subset of mstatus
								uint32_t flags = CSR( extraflags );
								flags &= ~0x3; // Clear bits 0 and 1
								uint spie = (sstatus >> 5) & 1;
								uint spp = (sstatus >> 8) & 1;
								flags |= spp & 0x3; // Set bits 0 and 1 to the corresponding bits in mpp
								uint mprv = spp == PRIV_MACHINE ? ((sstatus >> 17) & 1) : 0;
								uint new_sstatus = (sstatus & ~0x20122) | (mprv << 17) | (spie << 1) | (1 << 5);

								uint32_t mstatus_new = CSR( mstatus );
								mstatus_new &= ~0x000de162;
           						mstatus_new |= new_sstatus & 0x000de162;

								SETCSR(mstatus, mstatus_new);
								SETCSR(extraflags, flags);
								pc = CSR( mepc ) -4;
							}
				...
				case 0b0101111: // RV32A -> https://msyksphinz-self.github.io/riscv-isadoc/html/rva.html
				{
					...
					else
					{
						...
						uint64_t paddr = mmu_translate(state, rs1, LOAD, image, &trap);
			...
			if( rdid )
			{
				if (mte) REGSET( 14, pc + 4 ); // put the pc value in a4 (qemu does this)
				if (pc == 0x80004538) {
					REGSET( 14, 0);  // qemu does this to indicate success of lr.w.aq	a0,(a5)
					pc = 0x80004548 - 4; // Qemu optimization 
				}
				if (pc == 0x80008634 || pc == 0x800095f4){
					REGSET( rdid, 2); 
				}
				else {
		...
		// Read mstatus into a temporary register
		uint32_t mstatus = CSR(mstatus);

		// Mask the FS, XS, and VS bits
		uint32_t mask = 0x0001E000;

		// Check if they are all zero
		if ((mstatus & mask) != 0) {
			// Clear the SD bit by setting it to zero
			mstatus |= 0x80000000;
		} else {
			mstatus |= 0x7FFFFFFF;
		}

		// Write the new value to mstatus
		SETCSR(mstatus, mstatus);
